...
//     };
//   }, [authChecked, fetchDevices, setupEventSource]);
...
export default function DeviceDashboard() {
  ...
  // Move authentication check to a separate effect to avoid early returns
  useEffect(() => {
    const checkAuth = () => {
      const jwtToken = localStorage.getItem('jwt');
      if (!jwtToken) {
  ...
  // Main effect for fetching data - only runs after auth is checked
  useEffect(() => {
